...
namespace CluedIn.Provider.Trinet.Unit.Test.TrinetProvider
{
    public class GetCrawlJobDataBehaviour : TrinetProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(TrinetConstants.KeyName.ApiKey, nameof(TrinetCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task TrinetCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Trinet.Core.TrinetCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
