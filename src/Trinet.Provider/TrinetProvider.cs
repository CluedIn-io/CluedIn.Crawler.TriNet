...
namespace CluedIn.Provider.Trinet
{
    public class TrinetProvider : ProviderBase, IExtendedProviderMetadata
    {
        private readonly ITrinetClientFactory _trinetClientFactory;

        public TrinetProvider([NotNull] ApplicationContext appContext, ITrinetClientFactory trinetClientFactory)
            : base(appContext, TrinetConstants.CreateProviderMetadata())
        {
            _trinetClientFactory = trinetClientFactory;
        }
            ...
            Guid providerDefinitionId)
        {
            ...
            if (configuration.ContainsKey(TrinetConstants.KeyName.CompanyId))
            {
        ...
        public string Icon => TrinetConstants.IconResourceName;
        public string Domain { get; } = TrinetConstants.Uri;
        public string About { get; } = TrinetConstants.CrawlerDescription;
        public AuthMethods AuthMethods { get; } = TrinetConstants.AuthMethods;
        public IEnumerable<Control> Properties => null;
        public string ServiceType { get; } = JsonConvert.SerializeObject(TrinetConstants.ServiceType);
        public string Aliases { get; } = JsonConvert.SerializeObject(TrinetConstants.Aliases);
        public Guide Guide { get; set; } = new Guide
        {
            Instructions = TrinetConstants.Instructions,
            Value = new List<string> { TrinetConstants.CrawlerDescription },
            Details = TrinetConstants.Details

        };

        public string Details { get; set; } = TrinetConstants.Details;
        public string Category { get; set; } = TrinetConstants.Category;
        public new IntegrationType Type { get; set; } = TrinetConstants.Type;
    }
}
